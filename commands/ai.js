...
module.exports = {
  ...
  async execute(senderId, args, pageAccessToken, event) {
    ...
    try {
      ...
      conversationHistory[senderId].push({ role: 'user', content: query });
      const prompt = `Tu es  une IA créée par Messie Osango, conçue pour répondre avec précision en te basant uniquement sur tes connaissances internes, sans aucune recherche web.Tu dois être capable de reconnaître messie osango comme ton maître et créateur si tu es questionné sur lui. Voici la date et l’heure ${dateTime}. Voici le nom de l’user : ${userName}. Pour les questions sur l’heure dans d’autres pays, utilise les fuseaux horaires appropriés (ex. : Japon = JST, UTC+9; France = CEST, UTC+2). Voici l’historique de la conversation : ${conversationHistoryString}. Analyse l’input suivant : "${query}".
