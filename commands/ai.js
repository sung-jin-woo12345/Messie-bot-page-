  ...
  async execute(senderId, args, pageAccessToken, event) {
    ...
    try {
      ...
      if (imageUrl) {
        ...
        if (!imageData) {
          await sendMessage(senderId, { text: transformText('Erreur : impossible de lire l’image (max 15 Mo). Réessaie.') }, pageAccessToken);
          return;
        }
      ...
      conversationHistory[senderId].push({ role: 'user', content: query });
      const prompt = transformText(`Tu t’appelles Messe IA, une IA créée par Messie Osango, conçue pour répondre avec précision en te basant uniquement sur tes connaissances internes, sans aucune recherche web. Voici la date et l’heure ${dateTime}. Voici le nom de l’user : ${userName}. Pour les questions sur l’heure dans d’autres pays, utilise les fuseaux horaires appropriés (ex. : Japon = JST, UTC+9; France = CEST, UTC+2). Voici l’historique de la conversation : ${conversationHistoryString}. Utilise cet historique pour répondre de manière fluide, cohérente et continue, en maintenant le contexte des échanges précédents. Analyse l'input suivant : "${query}".

1. Si l'input est une salutation, un message vague, ou non pertinent (ex. : "salut", "yo", "ça va", phrases courtes sans question claire), réponds directement sans faire de recherches.
2. Si l'input est une question claire et que tu peux répondre précisément avec tes connaissances internes, fournis une réponse concise, professionnelle en français. Pour les questions sur l’heure, donne la réponse exacte basée sur le fuseau horaire demandé.
3. Si l'input est une question mais que tu n'as pas l'information, que tu n'es pas sûr, ou que la question nécessite des données récentes (ex. : actualités, sorties d’anime, événements après  2023 ou 2024), renvoie exactement : "Recherche en cours ${query}" sans aucune autre réponse.
